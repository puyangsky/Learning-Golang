...
func newTestSystem(t *testing.T, numClients int, params *lsp.Params) *testSystem {
	ts := new(testSystem)
	ts.t = t
	ts.params = params
	...
	for i := 0; i < numTries && ts.server == nil; i++ {
		...
		ts.server, err = lsp.NewServer(port, params)
		if err != nil {
	...
	for i := range ts.clients {
		...
		ts.clients[i], err = lsp.NewClient(hostport, params)
		if err != nil {
