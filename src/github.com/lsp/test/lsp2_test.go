...
type windowTestSystem struct {
	...
	maxEpochs          int
	timeout            int
	clientMap          map[int]lsp.Client   // Map of connected clients.
...
func newWindowTestSystem(t *testing.T, mode windowTestMode, numClients, numMsgs int, params *lsp.Params) *windowTestSystem {
	ts := new(windowTestSystem)
	ts.t = t
	ts.exitChan = make(chan struct{})
	ts.clientMap = make(map[int]lsp.Client)
	...
	for i := 0; i < numTries && ts.server == nil; i++ {
		...
		ts.server, err = lsp.NewServer(port, ts.params)
		if err != nil {
...
func (ts *windowTestSystem) streamToServer(connID int, cli lsp.Client, sendMsgs []string) {
...
func (ts *windowTestSystem) readFromServer(connID int, cli lsp.Client, totalMsgs int, checkpoints ...int) {
...
func TestWindow1(t *testing.T) {
	newWindowTestSystem(t, doMaxCapacity, 1, 10, &lsp.Params{3, 500, 5}).
...
func TestWindow2(t *testing.T) {
	newWindowTestSystem(t, doMaxCapacity, 5, 25, &lsp.Params{3, 500, 10}).
...
func TestWindow3(t *testing.T) {
	newWindowTestSystem(t, doMaxCapacity, 10, 25, &lsp.Params{3, 500, 10}).
...
func TestWindow4(t *testing.T) {
	newWindowTestSystem(t, doScatteredMsgs, 1, 10, &lsp.Params{3, 1000, 20}).
...
func TestWindow5(t *testing.T) {
	newWindowTestSystem(t, doScatteredMsgs, 5, 10, &lsp.Params{3, 1000, 20}).
...
func TestWindow6(t *testing.T) {
	newWindowTestSystem(t, doScatteredMsgs, 10, 10, &lsp.Params{3, 1000, 20}).
